...
class Twint:
    def __init__(self, config):
        ...
        if guest_token is None:
            self.token.refresh()
            guest_token = self.token.config.Guest_token
        else:
            self.token.config.Guest_token = guest_token
    ...
    async def Feed(self):
        ...
        while True:
            ...
            if self.config.Debug:
                print(
                    response,
                    file=open("twint-last-request.log", "w", encoding="utf-8"),
                )
            ...
            try:
                ...
                exc_fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[
                    1
                ]
                ...
                logme.warning(__name__ + ":Twint:Feed:noData" + str(e))
                logme.warning(f"{exc_type}, {exc_fname}, {exc_tb.tb_lineno}")
                ...
                if consecutive_errors_count < self.config.Retries_count:
                    ...
                    delay = round(2 ** consecutive_errors_count, 1,)
                    ...
                    logme.warning("sleeping for {} secs\n".format(delay))
                    time.sleep(delay)
    ...
    async def favorite(self):
        ...
        for tweet in self.feed:
            ...
            try:
                ...
                t_url = tweet.find("span", {"class": "metadata"}).find("a")[
                    "href"
                ]
                tweet_dict["data-conversation-id"] = t_url.split("?")[0].split(
                    "/"
                )[-1]
                tweet_dict["username"] = (
                ...
                date_str = (
                    tweet.find("td", {"class": "timestamp"}).find("a").text
                )
    ...
    async def run(self):
        ...
        if (
            self.config.TwitterSearch
            and self.config.Since
            and self.config.Until
        ):
    ...
    async def Lookup(self):
        ...
        try:
            if (
                self.config.User_id is not None
